...
Shader "Yangrc/CloudShaderPPSv2"
{
	HLSLINCLUDE

		#include "Packages/com.unity.postprocessing/PostProcessing/Shaders/StdLib.hlsl"
		#include "MissingIncludes.cginc"
	...
	ENDHLSL
		...
		SubShader
		{
...
#if USE_HI_HEIGHT
			#include "./CloudHierarchicalRaymarchPPSv2.cginc"
#else
			#include "./CloudNormalRaymarchPPSv2.cginc"
#endif	
			...
			Interpolator vert (appdata v)
			{
				...
				float2 uv = (v.vertex.xy + 1.0) * 0.5;
#if UNITY_UV_STARTS_AT_TOP
				uv = uv * float2(1.0, -1.0) + float2(0.0, 1.0);
#endif

				o.vsray = (2.0 * uv - 1.0) * _ProjectionExtents.xy + _ProjectionExtents.zw;
				return o;
			}

			float4 frag(Interpolator i) : SV_Target
			{
				float4 worldPos = mul(unity_CameraToWorld,float4(i.vsray, 1, 1));
				worldPos /= worldPos.w;
			...
			Pass{
				...
				#include "./CloudShaderHelperPPSv2.cginc"
				//#include "UnityCG.cginc"
